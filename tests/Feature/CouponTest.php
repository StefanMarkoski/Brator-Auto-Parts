...
final class CouponTest extends TestCase
{
    ...
    public function test_the_coupon_admin_renders_and_counts_what_the_bar_is_showing(): void
    {
        $live = app(GenerateCouponAction::class)->execute(10);
        $off = app(GenerateCouponAction::class)->execute(30);
        $off->update(['is_active' => false]);

        // Nothing else rendered this page while logged in, so a broken column or a variable the
        // controller stopped passing would have shipped unnoticed.
        $response = $this->actingAs($this->admin())->get('/admin/coupons')->assertOk();

        $response->assertSee($live->code, false)->assertSee($off->code, false);
        // The count must agree with the bar itself, not with how many codes exist.
        $response->assertSee('Showing 1 offer', false);
    }

    public function test_the_top_bar_shows_the_free_delivery_threshold_when_no_code_is_live(): void
    {
        // An empty promo bar is a hole in the theme's design, so there is always a fallback.
    ...
    public function test_a_live_coupon_replaces_the_free_delivery_message(): void
    {
        $coupon = app(GenerateCouponAction::class)->execute(20, Money::fromMajor(5_000));

        // No second switch to flip: generating it live is enough to advertise it.
        $html = $this->get('/')->assertOk()->getContent();
    ...
    public function test_a_coupon_with_no_minimum_says_any_order(): void
    {
    ...
    public function test_switching_a_coupon_off_removes_it_from_the_bar(): void
    {
        ...
        /*
         ...
         | type it and be told it is invalid. Off is the only control, so it has to be complete.
        */
        $this->get('/')->assertOk()
    ...
    public function test_every_live_coupon_gets_its_own_line_newest_first(): void
    {
        $oldest = app(GenerateCouponAction::class)->execute(5);
        ...
        $middle = app(GenerateCouponAction::class)->execute(15);
        $middle->created_at = now()->subDay();
        $middle->save();

        $newest = app(GenerateCouponAction::class)->execute(25);

        $html = $this->get('/')->assertOk()->getContent();

        // All three, not just the latest campaign.
        foreach ([$oldest, $middle, $newest] as $coupon) {
            $this->assertStringContainsString($coupon->code, $html, "{$coupon->code} is missing from the bar.");
        }

        // Newest at the top, because the campaign you just started is the one you want read
        // first — and because an arbitrary order would reshuffle itself between requests.
        $this->assertLessThan(
            strpos($html, $middle->code),
            strpos($html, $newest->code),
            'The newest code must be the first line.'
        );
        $this->assertLessThan(
            strpos($html, $oldest->code),
            strpos($html, $middle->code),
            'Lines must run newest to oldest.'
        );

        $this->assertStringNotContainsString('FREE DELIVERY', $html);
    }

    public function test_the_bar_carries_one_call_to_action_however_many_offers(): void
    {
        app(GenerateCouponAction::class)->execute(5);
        app(GenerateCouponAction::class)->execute(15);
        app(GenerateCouponAction::class)->execute(25);

        $html = $this->get('/')->assertOk()->getContent();

        // Three offers, one "Start shopping". Repeating the link per line is noise, and the
        // theme's design has exactly one link in this bar.
        $this->assertSame(1, substr_count($html, 'Start shopping'));
    }

    public function test_a_generated_code_is_advertised_immediately(): void
    {
        $this->actingAs($this->admin())
            ->post('/admin/coupons', ['discount_percent' => 20])
            ->assertRedirect(route('admin.coupons.index'));
    ...
    public function test_switching_a_code_back_on_returns_it_to_the_bar(): void
    {
        $coupon = app(GenerateCouponAction::class)->execute(10);
        ...
        $this->get('/')->assertOk()->assertDontSee($coupon->code, false);

        $this->actingAs($this->admin())
            ->put("/admin/coupons/{$coupon->id}", ['is_active' => 1])
            ->assertRedirect();

        // One switch, both directions. Nothing else has to be remembered to get it back.
        $this->get('/')->assertOk()->assertSee($coupon->code, false);
    }

    public function test_a_code_read_off_the_bar_actually_works(): void
    {
        $product = $this->product(priceMajor: 1_000, stock: 10);
        app(GenerateCouponAction::class)->execute(10);
